...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"zelcashaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"zelcashaddress\"  (string, required) The Zelcash address to use for the private key.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zelcash address: ")+name_);
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zelcash address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue zc_sample_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    JSDescription samplejoinsplit(*pzelcashParams,
                                  pubKeyHash,
                                  anchor,
                                  {JSInput(), JSInput()},
                                  {JSOutput(), JSOutput()},
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "\nResult:\n"
            "\"zelcashaddress\"    (string) The new zaddr\n"
            "\nExamples:\n"
...
UniValue z_listaddresses(const UniValue& params, bool fHelp)
{
    ...
    UniValue ret(UniValue::VARR);
    std::set<libzelcash::PaymentAddress> addresses;
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    libzelcash::PaymentAddress zaddr;
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        libzelcash::PaymentAddress zaddr = pa.Get();
    } catch (const std::runtime_error&) {
